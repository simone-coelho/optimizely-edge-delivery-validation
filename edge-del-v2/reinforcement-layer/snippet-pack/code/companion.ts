...
function boot(): void {
  ...
  // Initial apply. Fast path: inline edge manifest if present
  // (`applyInitial`). Followed unconditionally by `applyForRoute`, which
  // re-derives ops from `window.optimizely.get('data')` — that is the
  // only signal a snippet-only deployment has, since it has no edge
  // manifest at all.
  adapter.whenReady(() => {
    applyInitial();
